...
{
	static const char* name() { return WshOrange " cl"; }
...
#define WSHCL_LOG(_contents) cllog << _contents
 ...
 */
static std::string wshCLErrorHelper(const char *msg, cl::Error const &clerr) {
...
void CLMiner::workLoop()
{
	...
	try {
		while (!shouldStop())
		{
			...
			// It takes some time because wshash must be re-evaluated on CPU.
			if (nonce != 0) {
				Result r = WshashAux::eval(current.seed, current.header, nonce);
	...
	{
		cwarn << wshCLErrorHelper("OpenCL Error", _e);
		if(s_exit)
			exit(1);
	}
}
...
)
{
	...
	uint64_t dagSize = wshash_get_datasize(_currentBlock);
...
{
	WshashAux::LightType light = WshashAux::light(seed);
	...
	try
	{
		...
		WSHCL_LOG("Platform: " << platformName);
		...
		if (devices.empty())
		{
			WSHCL_LOG("No OpenCL devices found.");
			return false;
		}
		...
		WSHCL_LOG("Device:   " << device.getInfo<CL_DEVICE_NAME>() << " / " << device_version);
		...
		if (clVer == "1.0" || clVer == "1.1")
		{
			...
			{
				WSHCL_LOG("OpenCL " << clVer << " not supported, but platform Clover might work nevertheless. USE AT OWN RISK!");
			}
			else
			{
				WSHCL_LOG("OpenCL " << clVer << " not supported - minimum required version is 1.2");
				return false;
			}
		}
		...
		uint64_t dagSize = wshash_get_datasize(light->light->block_number);
		uint32_t dagSize128 = (unsigned)(dagSize / WSHASH_MIX_BYTES);
		...
		// See libwshash-cl/CMakeLists.txt: add_custom_command()
		...
		addDefinition(code, "ACCESSES", WSHASH_ACCESSES);
		...
		try
		{
			...
			m_searchKernel = cl::Kernel(program, "wshash_search");
			m_dagKernel = cl::Kernel(program, "wshash_calculate_dag_item");
			cllog << "Writing light cache buffer";
			m_queue.enqueueWriteBuffer(m_light, CL_TRUE, 0, light->data().size(), light->data().data());
		}
		catch (cl::Error const& err)
		{
			cwarn << wshCLErrorHelper("Creating DAG buffer failed", err);
			return false;
		}
		// create buffer for header
		WSHCL_LOG("Creating buffer for header.");
		...
		WSHCL_LOG("Creating mining buffer");
	...
	{
		cwarn << wshCLErrorHelper("OpenCL init failed", err);
		if(s_exit)
			exit(1);
		return false;
	}
