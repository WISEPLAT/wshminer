/*
* Genoil's CUDA mining kernel for Wiseplat
* based on Tim Hughes' opencl kernel.
* thanks to sp_, trpuvot, djm34, cbuchner for things i took from ccminer.
*/

#include "wshash_cuda_miner_kernel.h"
#include "wshash_cuda_miner_kernel_globals.h"
...
)
{
	switch (parallelHash)
	{
		case 1: wshash_search <1> <<<blocks, threads, 0, stream >>>(g_output, start_nonce); break;
		case 2: wshash_search <2> <<<blocks, threads, 0, stream >>>(g_output, start_nonce); break;
		case 4: wshash_search <4> <<<blocks, threads, 0, stream >>>(g_output, start_nonce); break;
		case 8: wshash_search <8> <<<blocks, threads, 0, stream >>>(g_output, start_nonce); break;
		default: wshash_search <4> <<<blocks, threads, 0, stream >>>(g_output, start_nonce); break;
	}
...
__global__ void
wshash_calculate_dag_item(uint32_t start)
{
	...
	)
{
	...
	{
		wshash_calculate_dag_item <<<blocks, threads, 0, stream >>>(i * blocks * threads);
