...
{
	static const char* name() { return WshOrange " cu"; }
...
bool CUDAMiner::init(const h256& seed)
{
	try {
		...
		WshashAux::LightType light;
		light = WshashAux::light(seed);
	...
	unsigned dagCreateDevice)
{
	try
	{
		...
		uint64_t dagSize = wshash_get_datasize(_light->block_number);
		...
		set_constants(dag, dagSize128, light, lightSize64); //in wshash_cuda_miner_kernel.cu
		...
		if(dagSize128 != m_dag_size || !dag)
		{
			...
			if (!hostDAG)
			{
				if((m_device_num == dagCreateDevice) || !_cpyToHost){ //if !cpyToHost -> All devices shall generate their DAG
					...
					wshash_generate_dag(dagSize, s_gridSize, s_blockSize, m_streams[0], m_device_num);
	...
	const dev::wsh::WorkPackage& w)
{
	...
	while (true)
	{
		...
		run_wshash_search(s_gridSize, s_blockSize, stream, buffer, m_current_nonce, m_parallelHash);
		if (m_current_index >= s_numStreams)
		{
			if (found_count)
				for (uint32_t i = 0; i < found_count; i++)
					...
					{
						Result r = WshashAux::eval(w.seed, w.header, nonces[i]);
