#include "WshStratumClient.h"
#include <libdevcore/Log.h>
#include <libwshash/endian.h>
...
WshStratumClient::WshStratumClient(int const & worktimeout, string const & email, bool const & submitHashrate) : PoolClient(),
	m_socket(nullptr),
...
WshStratumClient::~WshStratumClient()
{
	m_io_service.stop();
...
void WshStratumClient::connect()
{
	...
	m_resolver.async_resolve(q, boost::bind(&WshStratumClient::resolve_handler,
...
void WshStratumClient::disconnect()
{
	m_worktimer.cancel();
...
void WshStratumClient::resolve_handler(const boost::system::error_code& ec, tcp::resolver::iterator i)
{
	...
	if (!ec)
	{
		...
		async_connect(*m_socket, i, end, boost::bind(&WshStratumClient::connect_handler,
						this, boost::asio::placeholders::error,
						boost::asio::placeholders::iterator));
	}
	else
...
void WshStratumClient::reset_work_timeout()
{
	m_worktimer.cancel();
	m_worktimer.expires_from_now(boost::posix_time::seconds(m_worktimeout));
	m_worktimer.async_wait(boost::bind(&WshStratumClient::work_timeout_handler, this, boost::asio::placeholders::error));
}

void WshStratumClient::async_write_with_response()
{
	if (m_connection.SecLevel() != SecureLevel::NONE) {
		async_write(*m_securesocket, m_requestBuffer,
			boost::bind(&WshStratumClient::handleResponse, this,
...
void WshStratumClient::connect_handler(const boost::system::error_code& ec, tcp::resolver::iterator i)
{
	(void)i;
	...
	if (!ec)
	{
		...
		switch (m_connection.Version()) {
			case WshStratumClient::STRATUM:
				m_authorized = true;
				os << "{\"id\": 1, \"method\": \"mining.subscribe\", \"params\": []}\n";
				break;
			case WshStratumClient::WSHPROXY:
				...
				if (m_email.empty())
				{
					os << "{\"id\": 1, \"worker\":\"" << m_worker << "\", \"method\": \"wsh_submitLogin\", \"params\": [\"" << user << "\"]}\n";
				}
				else
				{
					os << "{\"id\": 1, \"worker\":\"" << m_worker << "\", \"method\": \"wsh_submitLogin\", \"params\": [\"" << user << "\", \"" << m_email << "\"]}\n";
				}
				break;
			case WshStratumClient::WISEPLATSTRATUM:
				m_authorized = true;
				os << "{\"id\": 1, \"method\": \"mining.subscribe\", \"params\": [\"wshminer/" << wshminer_get_buildinfo()->project_version << "\",\"WiseplatStratum/1.0.0\"]}\n";
				break;
		}
...
void WshStratumClient::readline() {
	x_pending.lock();
	if (m_pending == 0) {
		if (m_connection.SecLevel() != SecureLevel::NONE) {
			async_read_until(*m_securesocket, m_responseBuffer, "\n",
				boost::bind(&WshStratumClient::readResponse, this,
...
void WshStratumClient::handleHashrateResponse(const boost::system::error_code& ec) {
	(void)ec;
}

void WshStratumClient::handleResponse(const boost::system::error_code& ec) {
	if (!ec)
	{
		readline();
	}
...
void WshStratumClient::readResponse(const boost::system::error_code& ec, std::size_t bytes_transferred)
{
	dev::setThreadName("stratum");
	x_pending.lock();
	...
	if (!ec && bytes_transferred)
	{
		...
		else if (m_connection.Version() != WshStratumClient::WSHPROXY)
		{
...
void WshStratumClient::processExtranonce(std::string& enonce)
{
...
void WshStratumClient::processReponse(Json::Value& responseObject)
{
	...
	{
		case 1:
		if (m_connection.Version() == WshStratumClient::WISEPLATSTRATUM)
		{
		...
		if (m_connection.Version() != WshStratumClient::WSHPROXY)
		{
	...
	default:
			...
			if (params.isArray())
			{
				...
				if (m_connection.Version() == WshStratumClient::WISEPLATSTRATUM)
				{
					...
					if (sHeaderHash != "" && sSeedHash != "")
					{
						...
						m_current.startNonce = wshash_swap_u64(*((uint64_t*)m_extraNonce.data()));
		...
		else if (method == "mining.set_difficulty" && m_connection.Version() == WshStratumClient::WISEPLATSTRATUM)
		{
		...
		else if (method == "mining.set_extranonce" && m_connection.Version() == WshStratumClient::WISEPLATSTRATUM)
		{
			params = responseObject.get("params", Json::Value::null);
			if (params.isArray())
			{
		...
		{
			os << "{\"error\": null, \"id\" : " << id << ", \"result\" : \"" << wshminer_get_buildinfo()->project_version << "\"}\n";
			async_write_with_response();
		}
		break;
	}
...
void WshStratumClient::hashrate_event_handler(const boost::system::error_code& ec)
{
	if (ec || m_linkdown) {
		return;
	}
	...
	string json = "{\"id\": 6, \"jsonrpc\":\"2.0\", \"method\": \"wsh_submitHashrate\", \"params\": [\"" + m_rate + "\",\"0x" + this->m_submit_hashrate_id + "\"]}\n";
	std::ostream os(&m_requestBuffer);
	os << json;

	if (m_connection.SecLevel() != SecureLevel::NONE)
		async_write(*m_securesocket, m_requestBuffer,
			boost::bind(&WshStratumClient::handleHashrateResponse, this, boost::asio::placeholders::error));
	else
		async_write(*m_socket, m_requestBuffer,
			boost::bind(&WshStratumClient::handleHashrateResponse, this, boost::asio::placeholders::error));
}

void WshStratumClient::work_timeout_handler(const boost::system::error_code& ec) {
	if (!ec) {
...
void WshStratumClient::response_timeout_handler(const boost::system::error_code& ec) {
	if (!ec) {
...
void WshStratumClient::submitHashrate(string const & rate) {
	...
	m_hashrate_event.async_wait(boost::bind(&WshStratumClient::hashrate_event_handler, this, boost::asio::placeholders::error));
}

void WshStratumClient::submitSolution(Solution solution) {
	...
	switch (m_connection.Version()) {
		...
		case WshStratumClient::WSHPROXY:
			json = "{\"id\": 4, \"worker\":\"" +
				m_worker + "\", \"method\": \"wsh_submitWork\", \"params\": [\"0x" +
				nonceHex + "\",\"0x" + solution.work.header.hex() + "\",\"0x" +
				solution.mixHash.hex() + "\"]}\n";
			break;
		case WshStratumClient::WISEPLATSTRATUM:
	...
	m_responsetimer.async_wait(boost::bind(&WshStratumClient::response_timeout_handler, this, boost::asio::placeholders::error));
}
