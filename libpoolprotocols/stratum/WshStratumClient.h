...
class WshStratumClient : public PoolClient
{
public:

	typedef enum { STRATUM = 0, WSHPROXY, WISEPLATSTRATUM } StratumProtocol;

	WshStratumClient(int const & worktimeout, string const & email, bool const & submitHashrate);
	~WshStratumClient();
...
private:
	...
	string m_worker; // wsh-proxy only;
