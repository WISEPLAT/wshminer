...
inline std::ostream& operator<<(std::ostream& _out, WorkingProgress _p)
{
	...
	_out << "Speed "
		 << WshTealBold << std::fixed << std::setw(6) << std::setprecision(2) << mh << WshReset
		 << " Mh/s    ";

	for (size_t i = 0; i < _p.minersHashes.size(); ++i)
	{
		mh = _p.minerRate(_p.minersHashes[i]) / 1000000.0f;
		_out << "gpu/" << i << " " << WshTeal << std::fixed << std::setw(5) << std::setprecision(2) << mh << WshReset;
		if (_p.minerMonitors.size() == _p.minersHashes.size())
			_out << " " << WshTeal << _p.minerMonitors[i] << WshReset;
		_out << "  ";
	}
