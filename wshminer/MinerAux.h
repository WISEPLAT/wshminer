...
#include <libwshcore/Exceptions.h>
#include <libdevcore/SHA3.h>
#include <libwshcore/WshashAux.h>
#include <libwshcore/Farm.h>
#include <wshminer-buildinfo.h>
#if WSH_WSHASHCL
#include <libwshash-cl/CLMiner.h>
#endif
#if WSH_WSHASHCUDA
#include <libwshash-cuda/CUDAMiner.h>
#endif
#include <libpoolprotocols/PoolManager.h>
#include <libpoolprotocols/stratum/WshStratumClient.h>
#include <libpoolprotocols/getwork/WshGetworkClient.h>
...
{
	static const char* name() { return WshGreen "  m"; }
...
public:
	...
	bool interpretOption(int& i, int argc, char** argv)
	{
		...
		else if ((arg == "-SP" || arg == "--stratum-protocol") && i + 1 < argc)
		{
			deprecated(arg);
			try {
				m_endpoints[k_primary_ep_ix].Version((WshStratumClient::StratumProtocol)atoi(argv[++i]));
...
#endif
#if WSH_WSHASHCL || WSH_WSHASHCUDA
	...
	void execute()
	{
		...
		auto* build = wshminer_get_buildinfo();
		minelog << "wshminer version " << build->project_version;
...
#else
			cerr << "Selected GPU mining without having compiled with -DWSHASHCL=1" << endl;
			exit(1);
#endif
		}
		if (m_minerType == MinerType::CUDA || m_minerType == MinerType::Mixed)
...
#else
			cerr << "CUDA support disabled. Configure project build with -DWSHASHCUDA=ON" << endl;
			exit(1);
#endif
		}
	...
	static void streamHelp(ostream& _out)
	{
		_out
			...
			<< "        0: official stratum spec: wshpool, wisemine, coinotron, mph, nanopool (default)" << endl
			<< "        1: wsh-proxy compatible: dwarfpool, f2pool, nanopool (required for hashrate reporting to work with nanopool)" << endl
			<< "        2: WiseplatStratum/1.0.0: nicehash" << endl
			...
			<< "    -SE, --stratum-email <s> Email address used in wsh-proxy (optional)" << endl
			...
			<< "        Example: stratum+ssl://0x012345678901234567890234567890123.miner1@wisemine.org:5555" << endl
			<< endl
...
private:
	...
	void doMiner()
	{
		...
		if (m_mode == OperationMode::Stratum) {
			client = new WshStratumClient(m_worktimeout, m_email, m_report_stratum_hashrate);
		}
		else if (m_mode == OperationMode::Farm) {
			client = new WshGetworkClient(m_farmRecheckPeriod);
		}
